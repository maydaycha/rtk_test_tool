...
scan_storage_device() {
    while [ "1" == "1" ]
    do
        mnt_point1=$( mount | grep usb | grep sda | awk '{print $3}' )
        mnt_point2=$( mount | grep usb | grep sdb | awk '{print $3}' )
...
main() {
    ...
    ## use dd to generate 1GB file to mnt_point1
    dd if=/dev/urandom of="$mnt_point1/usb_md5_test_file" bs=16k count=64k
    echo "generate 1GB MB file"
